...
def extract_resource_data(resource_results) -> Optional[Dict[str, Any]]:
    if not resource_results:
        return None
    try:
        if not resource_results.contents:
            return None
        
        raw = resource_results.contents[0].content
        ...
        print("Erro ao extrair dados do recurso:", e)
        return None

@mcp.resource("conta://{cpf}")
async def obter_conta(cpf:str):
    cpf = cpf.strip()
    data = contas_mdbank.get(cpf, {"erro": "Conta nao encontrada"})
...
def abrir_conta_prompt(nome: str, cpf: str):
    return [
        Message("O cliente deseja abrir uma conta."),
...
@mcp.prompt
def abrir_cartao_prompt(cpf: str, tipo: str):
    return [
...
@mcp.tool
async def obter_conta(cpf: str, ctx: Context):
...
@mcp.tool
async def obter_cartao(cpf: str, ctx: Context):
...
async def solicitar_cartao(cpf: str, tipo: str, ctx: Context):
    cpf = cpf.strip()
    await ctx.info(f"[Cartão] Processando CPF: {cpf}")
    resource = await ctx.read_resource(f"conta://{cpf}")
    ...
    if not conta or "erro" in conta:
        return {
            "status": "erro",
            "cartao": "Cliente nao possui conta"
        }
    
    resource = await ctx.read_resource(f"cartao://{cpf}")

    cartao_existente = extract_resource_data(resource)
    ...
    numero_cartao = random.randint(10000, 99999)
